...
test_that("nf_gff_study_summary_module_server", {
  shiny::testServer(
    ...
    ),
    {
      ...
      expect_type(filtered_data()$tables, "list")
      expect_named(filtered_data()$tables, c('files', 'publications', 'studies', 'tools', 'incoming_data'))
      expect_equal(filtered_data()$selected_group, "GFF")
      expect_equal(filtered_data()$selected_study, "Ataluren-promoted therapeutic nonsense suppression for neurofibromatosis")
      expect_true(nrow(filtered_data()$tables$incoming_data) == 0)
